...
class Generation {
  ...
  /**
   * Initalize the Generation with citizens
   * @param {object}
   */

  initialize(Citizen) {
    for (let i = 0; i < this.population; i++) {
      let new_citizen = new Citizen({
        id: i,
      });
      this.species.push(new_citizen);
    }
  }

  /**
   * Picks one citizen from the population
   * @returns A citizen
   */
  ...
  evolve() {
    // Store High Score
    this.generation += 1;
    let gen_highscore = Math.max.apply(
      Math,
      this.species.map((o) => o.score)
    );
    this.high_score =
      gen_highscore > this.high_score ? gen_highscore : this.high_score;

    // Calculate Total Score of this Generation
    let total_score = 0;
    this.species.forEach((citizen) => {
      total_score += citizen.score;
    });
